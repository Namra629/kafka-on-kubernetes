# confluent-kafka-on-kubernetes ( Not Kraft Controller but Confluent Operator installing through helm )
...
3.  For PVC to bound to a PV, storage class is necessary.

<img width="530" height="40" alt="image" src="https://github.com/user-attachments/assets/5e46b950-5c0a-4768-bfe4-c9c65451d97a" />
  
4.Run the CSI driver openebs scripts to install the CSI driver and deploy the custom storage class required for the PVC to bound to PV as the kubeadm kubernetes has no default dynamic (automatic) volume provsioning.
...
7.  A new sc will be made which will be used as default by kubernetes for all the pods in confluent namespace.

8. Now , all the kafka pods will be in the running state.

   <img width="800" height="300" alt="image" src="https://github.com/user-attachments/assets/6563dfdd-0eb4-4092-ac28-e062e79477f0" />


9. On master node,  Create a service file of control-center to expose it on external port. ( File present in the repository )
